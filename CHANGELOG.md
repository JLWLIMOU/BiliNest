...
- **更新只留一个按钮：点「更新到 vX」→ 确认 → 拉取并重启服务 → 页面自动刷新**（原来把"下载安装包 / 拉取源码 / 打开 Releases"三个入口都摆在设置里，用户不知道该点哪个）。
  - 设置 →「关于」现在只有一个按钮（平时显示"检查更新"，检查到新版本时变成"更新到 vX"）+ 一个"检查方式"下拉：**自动（打开页面时）** / **仅手动**（新增 `updateCheck` 设置项）。
  - 点什么、发生什么：
  ...
  - README 其余部分同步补齐：新增**播放页截图**（`docs/img/player.png`，与主页截图一样是演示数据）、功能一览里的「版本更新」与播放快捷键、英文区的 updates 说明、目录结构里的 `bin/` 与 `docs/`、FAQ 里"怎么更新"一条，以及一个自动显示最新版本号的 Release 徽章。
  - 顺带修一个截图暴露出来的小问题：选集面板的 CSS 兜底高度 `calc(100vh - 顶栏 - 40px)` **比它的实际剩余空间大**（面板的自然顶部在返回键那一行下面，约 128px），超过画面高度时底部会被视口切掉、看起来"没闭合"。正常路径由 `sizeEpisodePanel()` 覆盖成"与画面等高"，现在兜底值也改保守了（`100dvh - 顶栏 - 104px`）。
  ...
  - ⚠️ 两个实测踩到的坑（都已修，留档）：
    ...
    - 前端"等服务回来"的判断太宽松：旧进程还在应答就返回成功 → 页面在**旧服务**上刷新了一遍，看起来更新成功、版本却没变。现在必须**先看到一次请求失败**或**版本号真的变了**才算重启完成。
