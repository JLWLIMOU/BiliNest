...
- **按 Apple 设计思路做的一轮视觉/材质改造**（功能与逻辑不变，改动集中在样式与少量渲染代码）。
  ...
  - 技术细节：
    ...
    - **顶栏改成滚动边缘**：分隔线默认透明，滚动超过 4px 才浮现（Apple §12：悬浮玻璃不该被一条常驻硬线说成"固定的条"）。实测 顶部 `rgba(0,0,0,0)` → 滚动后 `rgb(230,230,225)`；
  ...
  - 说明：这一轮**刻意没有**动布局结构与主色——Apple 的做法是先让层级、材质与反馈正确，再谈装饰。
...
- **视觉再进一步：环境光 / 卡片材质 / 续播主视觉 / 首屏入场**（功能与逻辑不变）。
  ...
  - 技术细节：
    ...
    - **悬浮反馈改为"封面推近"**：卡片不再整体位移，改为封面在图框内 `scale(1.04)`（Apple TV 的焦点感），配合阴影加深；
    ...
    - 搜索框与排序控件改为"填充式"（Apple 习惯：控件自己有底色，聚焦时才浮起 + 焦点环），搜索框加左侧放大镜；
...
- **按 Apple 的理念重做视觉层：分段控件 / 无卡片的图墙 / 文字压在画面上**（数据与交互逻辑不动，改的是渲染出来的结构与样式）。
  - 涉及文件：`public/styles.css`、`public/app.js`、`docs/img/dashboard.png`
  - 技术细节：
    - **标签栏改成分段控件**（Apple 切换视图的标准形态）：一个填充的圆角容器 + 一块会滑的白色滑块，宽度按内容收缩、不再整行铺开；滑块跟着选中的那一段走（位移 + 宽度，弹簧曲线 `--ease-spring`），自定义标签的滑块带一点绿以保留"源 / 库"的语义；选中态用"文字变成满对比度"表达，颜色标记交给滑块。滑块几何由 `syncTabIndicator()` 输出 `translateX + width`（不再用 `scaleX` 拉伸，那会把圆角拉变形）；
    - **网格去掉"卡片"**：`.card` 不再有描边、底色和卡片阴影，画面本身成为内容、文字直接坐在页面底色上；层次由画面自己的阴影承担；悬浮时"画面浮起来"（阴影加深 + 画面推近 `scale(1.04)`，用 `--ease-spring-soft`），卡片本身不位移；
    - **续播卡把文字压在画面上**（Apple TV 的做法）：标题、进度、时间与"继续学习 · 剧集"角标都坐在画面里，底部一条渐变保证可读；进度条改成细白线（比彩色渐变更像系统组件）；点击仍然交给整张卡（文字层 `pointer-events: none`）；
...
- **按 Apple 的语义色重做配色，并把卡片恢复成"表面"**（功能与逻辑不变）。
  - 涉及文件：`public/styles.css`、`public/app.js`、`docs/img/dashboard.png`
  - 技术细节：
    - **配色换成 Apple 的系统色**：页面底 `systemGroupedBackground`(#f2f2f7) + 卡片 `systemBackground`(#fff)——"白卡片浮在浅灰底上"正是 Apple 分组列表/网格的形态；文字只留两级（label #1c1c1e / secondaryLabel #8e8e93），层级靠字重与字号而不是多套颜色；强调色换成 Apple 蓝（浅色 #0071e3 / 深色 #0a84ff）；
    - **分隔线与控件填充改为半透明**：`--separator: rgba(60,60,67,.13)`、`--fill: rgba(120,120,128,.12)`（Apple 的 separator / systemFill），并把 `--border` 指向 `--separator`——于是所有既有的 `1px solid var(--border)` 自动变成能适配任何底色的半透明分隔；搜索框、排序、表单控件改用 `--fill` 填充；
    ...
    - **标签栏改成"浮动玻璃"**：分段控件加 `position: sticky`（吸在顶栏下方），背景改为半透明 + `backdrop-filter` 模糊，滚动时它浮在内容之上而不随页面滚走（Apple 的浮动 chrome）；`prefers-reduced-transparency` 下退化为实心。
