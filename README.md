...
- **以收藏夹创建标签页**：收藏夹卡片「⋯」→「以此收藏夹创建标签页」，会把该收藏夹里的视频装进一个新的自定义标签页（名称带 ★ 前缀，如「★ 动画参考」）。创建时可选**是否同时把视频加入视频库**（默认不勾）：不勾就只在标签页里（视频库不变），勾了才逐个入库。标签页内这类"未入库"的视频卡片底部会标「未加入视频库」，「⋯」里可随时「加入视频库 / 从本标签页移除」。标签页是当时的快照，收藏夹之后新增的内容不会自动同步。
