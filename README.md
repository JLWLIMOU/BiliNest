...
- **纯净播放**：应用内置自研播放器——用应用内的登录态获取 B 站 DASH 自适应码率流，**画质切换由 dash.js 自动完成，永不跳转 B 站官网**；弹幕显示（**无弹幕输入框，不能发送弹幕**）、CC 字幕、**播放倍速**、播放/暂停/进度/音量/全屏均支持。**快捷键**：空格播放/暂停、←→ 快退快进、↑↓ 音量、F 全屏 —— 进播放页即可用，不必先点一下播放器。播放地址服务暂不可用时自动降级为官方嵌入播放器。
