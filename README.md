...
- **没有封面就生成一张**：本地视频、本地文件夹列表、以及 B 站封面缺失的**视频**条目，卡片封面会按名字生成同色系柔光渐变海报，标题自动缩放铺在上面；名字太长放不下时退化成"首字大字"，完整标题回到卡片下方。颜色由名字决定，同一条内容每次都是同一个色。**收藏夹不参与海报**（有 B 站封面就带一枚「收藏夹」角标，没有就显示中性瓦片 + 收藏图标），免得收藏夹库和视频库长得一样。
