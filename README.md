# BiliNest · 无干扰 B站（Bilibili）学习播放器
...
- **主页（仪表盘）**：标签页形式展示「继续学习 / 视频库 / 收藏夹库 / 学习 UP主」四个系统标签；收藏夹可在内容源中一键「加入收藏夹库」，收藏夹里的单个视频也可直接「+ 添加到视频库」。
...
- **搜索**：搜索框就在大标题行里，实时筛选当前栏目（视频库 / 自定义标签页），并显示命中数（如 `3 / 24 个视频`）；内容源弹窗里可以搜收藏夹名，也能搜到收藏夹**里面的视频**（命中后可直接播放 / 加入视频库，或点进去定位）；收藏夹视图内可搜索当前收藏夹的视频。
- **栏位与二级页**：各标签默认只显示前几个，超出后点「展开全部」进入二级浏览页，支持翻页、排序与搜索（不再横向滚动）。
- **纯净播放**：应用内置自研播放器——用应用内的登录态获取 B 站 DASH 自适应码率流，**画质切换由 dash.js 完成（默认清晰度可在设置里指定，控制条上随时手动切换），永不跳转 B 站官网**；弹幕显示（**无弹幕输入框，不能发送弹幕**）、CC 字幕、**播放倍速**、播放/暂停/进度/音量/全屏均支持。**快捷键**：空格播放/暂停、←→ 快退快进、↑↓ 音量、F 全屏 —— 进播放页即可用，不必先点一下播放器。播放地址服务暂不可用时自动降级为官方嵌入播放器。
- **播放倍速**：控制条上直接选档位（0.5× / 0.75× / 1.0× / 1.25× / 1.5× / 1.75× / 2.0×），不是 1.0× 时按钮会亮成蓝色胶囊，一眼看得出当前没在常速；选择会被记住，换集、切清晰度、下次打开都还是这个速度。
- **默认清晰度**：`设置 → 播放` 里可以选 **高 / 中 / 低 / 自动**，打开每个视频时按它选一档 —— 高 = 最高可用画质；中 = 最接近 480P 的那一档；低 = 最低那一档；自动 = 交给播放器按网速实时决定（默认）。播放页控制条上的「画质」随时可以手动改，改的是这一个视频、不影响默认值。
- **弹幕 / 字幕也能设默认**：`设置 → 播放` 里可以定「默认显示弹幕」「默认开启字幕」，以及字幕的**字号（小 / 中 / 大 / 特大）和位置（贴底 / 中间 / 最高）**—— 和播放页控制条上的「弹」「字幕」「Aa」是同一份设置，改完正在播的视频也会立刻跟着变。没字幕的视频不会有任何多余提示。
- **选集**：支持多 P 视频与 UP 主合集（视频系列）的选集切换。
- **单个视频**：粘贴视频链接 / BV 号 / av 号即可添加。
- **本地视频 / 本地文件夹列表**：一个入口「添加本地视频 / 文件夹…」——选文件就是单视频，选文件夹会**自动解析里面的视频合成一个列表**（按文件名自然排序，卡片用文件夹名，播放页右侧按文件名选集），和 B 站合集是同一种用法。只收浏览器原生能播的格式（mp4 / m4v / mov / webm / mkv），添加时顺便读出时长。Chromium 内核浏览器可跨会话保留文件权限，其余浏览器本次会话可播放。
- **自定义标签页**：标签栏末尾的「＋」直接新建标签页（建完可内联改名；双击标签改名；按住标签拖动排序，拖动时其余标签实时让位；标签过多时横向滚动，系统标签固定在左侧）。空标签页里的「＋ 添加内容」可从 **源收藏夹 / 视频库 / 收藏夹库 / 学习 UP主** 四处挑选（库侧条目带封面 / 头像），也可以直接**粘贴 B 站链接**（同时加入视频库）；从里面进收藏夹挑视频时，返回键会回到**上一级的选择器**（分区、搜索词、勾选都还在），不用从头点一遍。卡片右上角「⋯」→「从本标签页移除」时可选是否连库内一并删除（默认不删）。标签页默认只存引用，**删标签页不会删内容**；只有删除标签页时主动勾选「同时删除视频库里的这些视频」才会连库一起删（见下条）。
- **学习 UP主**：在「学习 UP主」标签输入 UID 即可收藏 UP主（头像 / 昵称 / 简介 / 粉丝数 / 星级），点卡片跳转其 B 站主页。
- **视频卡片改名（右上角 ⋯）**：视频卡片的右上角是「⋯」，里面是「重命名 /（恢复原名）/ 删除」——删除也收进来了，右上角不再单独放一个 ✕；右键视频卡片同样能打开。改名是就地编辑（回车保存、Esc 取消），只改本地显示，源站标题不动。收藏夹卡片也是「⋯」（菜单：以此收藏夹创建标签页 / 从收藏夹库移除），但**不参与改名**；学习 UP主 / 继续学习的卡片保持原来的 ✕。
- **以收藏夹创建标签页**：收藏夹卡片「⋯」→「以此收藏夹创建标签页」，会把该收藏夹里的视频装进一个新的自定义标签页（名称带 ★ 前缀，如「★ 动画参考」）。创建时可选**是否同时把视频加入视频库**（默认不勾）：不勾就只在标签页里（视频库不变），勾了才逐个入库。标签页内这类"未入库"的视频卡片右下角标「未加入视频库」，**星级照常能打**（记在标签页里），「⋯」里可随时「加入视频库 / 从本标签页移除」。

  这类标签页会**镜像**源收藏夹：每次切过去时查一次 —— 源里新增的自动补进来，源里删掉的在标签页里跟着移除；**你自己加进这个标签页的内容不受影响**（不会被覆盖，也不会因为收藏夹变化而消失）。往标签页里加内容同样不会改动收藏夹库和源收藏夹。
- **自定义标签页也能排序**：标签页标题行有排序下拉 —— 添加顺序（默认，条目在页里的先后）/ 发布时间 / 星级 / 播放量，每个标签页各记各的，配合搜索框一起筛。收藏夹标签页可以继续用「＋ 添加内容」往里加东西（从源收藏夹 / 视频库 / 收藏夹库 / 学习 UP主 选），原有内容不会被动。
- **删除标签页可以顺带清库**：删除自定义标签页时，确认框里可选「同时删除视频库里的这些视频（N 个）」，默认不勾；勾选只删"既在本标签页、又在视频库"的那些（只在本页、没入库的不受影响）。
...
- **检测 Node.js**（不自带——不让安装包为运行时白白多出 87MB）：找到 18 或更高版本就直接装；没找到（或版本过低）会出现「运行环境检查」页，二选一：打开 nodejs.org 自己装 / 先跳过。**安装包自己不下载、不执行任何东西** —— 只读文件版本信息和注册表来判断 Node，下载与安装都交给你的浏览器；
- 卸载时会先停掉正在跑的本地服务，再问一句是否连用户数据一起删（**默认保留**，重新安装后还能接着用）。

#### 杀毒软件报毒怎么办

这类"本地小工具 + 未签名安装包"被误报是常见情况，按代价从低到高：

1. **先用便携版**：Releases 里同时提供 `BiliNest-x.y.z-portable.zip`（解压即用，不走安装程序）。如果你的杀软只拦安装包，用这个最省事。
2. **提交误报**：把安装包的文件名、下载地址和 SHA-256 提交给杀软的误报反馈渠道（火绒：官网/论坛的误报反馈，或客户端隔离区右键上报）。厂商核对后会加进白名单，**所有用户都不再被拦** —— 这是最根本的免费办法。
3. **代码签名**：给安装包签上证书，能同时解决"未知发布者"和大部分云查杀误报。开源项目可以申请 Certum 的开源代码签名证书；商业 OV / EV 证书按年付费（EV 对 SmartScreen 立即生效）。
4. 校验文件：Releases 里每个文件都可以在页面上核对大小与 SHA-256，也可以在 [VirusTotal](https://www.virustotal.com/) 自行上传查看命中情况（我们不会代为上传）。
...
> 提示：**更新通道是 1.4.5 才彻底修好的**。1.4.3 / 1.4.4 虽然也能自动下载替换，但遇到代理、或遇到"能开 github.com 却下不动资产 CDN"的线路时仍会失败 —— **1.4.4 及更早的版本请先按下面「方式二 / 方式三」手动更新一次到最新版**（更新失败过的尤其如此），之后就都能一键更新了。1.4.5 起本地服务会自动使用环境变量代理 / Windows 系统代理，下载源被拦时还会自动改用另一个 GitHub 域名取更新包。
...
> 从安装版 / 便携版更新时，安装包会**先找一遍本机已装的 BiliNest**（注册表卸载信息 → 桌面快捷方式 → 常见安装位置 → 安装包自己所在的目录），找到就直接按「更新」处理：装回原目录、跳过目录选择页、只替换程序文件，并弹窗告诉你是"更新到 vX"而不是新装。检测到的是源码检出（含 `.git`）则不会覆盖，提示你用 `git pull`。安装包比已装的旧时会先问一句，静默安装则直接拒绝，不会把新版本悄悄降级。

> 发布说明最上面那段以 ⚠ 开头的文字会被当作**重要提醒**：`设置 → 关于` 的更新区里直接显示（不用展开），更新确认框里也会先念一遍；自动检查遇到带提醒的版本会弹一次提示（同一版本只提醒一次）。这段文字写在 release 正文里，所以**旧版本客户端也会把它显示在"这次更新了什么"中**。
...
2. **添加学习内容**：点右上角「内容源」→ 收藏夹列表点「加入收藏夹库」（可打星）；收藏夹内某视频点封面右上角「+」加入视频库；也可粘贴单个视频链接或选择本地视频。
...
**Q：开了梯子（代理），为什么应用内更新还是失败？**
因为 Node 自带的网络请求**默认不读系统代理**：浏览器走代理能打开 GitHub，本地服务却是直连，于是"检查更新"可能成功、"下载新版本"却超时。1.4.5 起本地服务会自己找代理并按这个顺序试：**环境变量代理（`HTTPS_PROXY` / `HTTP_PROXY` / `ALL_PROXY`）→ Windows 系统代理（注册表里 `ProxyEnable=1` 的那套）→ 直连**，哪条通就用哪条；全都失败时，错误信息会逐条说明每条通道失败的原因。

- 最常见的情况：代理软件开了"系统代理"开关就行，不用做别的（Clash / v2rayN 的"系统代理"就是写注册表那套）。
- 用的是**仅 SOCKS** 端口：请在代理软件里换用它的 **HTTP / 混合端口**（如 Clash 的 `mixed-port`），或设环境变量 `HTTPS_PROXY=http://127.0.0.1:端口`。
- 用 **PAC 脚本**：自动检测不到，建议直接在代理软件里切到"系统代理（全局/规则）"模式，或设置上面的环境变量。
- 代理本身连不上 GitHub：会自动退回直连再试一次，所以"代理坏了"不会连带把更新彻底堵死。
