...
- **自定义标签页**：标签栏末尾的「＋」直接新建标签页（建完可内联改名；双击标签改名；按住标签拖动排序，拖动时其余标签实时让位；标签过多时横向滚动，系统标签固定在左侧）。空标签页里的「＋ 添加内容」可从 **源收藏夹 / 视频库 / 收藏夹库 / 学习 UP主** 四处挑选（库侧条目带封面 / 头像）；从里面进收藏夹挑视频时，返回键会回到**上一级的选择器**（分区、搜索词、勾选都还在），不用从头点一遍。卡片右上角「⋯」→「从本标签页移除」时可选是否连库内一并删除（默认不删）。标签页默认只存引用，**删标签页不会删内容**；只有删除标签页时主动勾选「同时删除视频库里的这些视频」才会连库一起删（见下条）。
...
- **以收藏夹创建标签页**：收藏夹卡片「⋯」→「以此收藏夹创建标签页」，会把该收藏夹里的视频装进一个新的自定义标签页（名称带 ★ 前缀，如「★ 动画参考」）。创建时可选**是否同时把视频加入视频库**（默认不勾）：不勾就只在标签页里（视频库不变），勾了才逐个入库。标签页内这类"未入库"的视频卡片右下角标「未加入视频库」，**星级照常能打**（记在标签页里），「⋯」里可随时「加入视频库 / 从本标签页移除」。标签页是当时的快照，收藏夹之后新增的内容不会自动同步；**往标签页里加内容不会改动收藏夹库和源收藏夹**。
