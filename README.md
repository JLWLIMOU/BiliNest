...
- **本地视频 / 本地文件夹列表**：选单个文件就是单视频；选文件夹会**自动解析里面的视频合成一个列表**（按文件名自然排序，卡片用文件夹名，播放页右侧按文件名选集）—— 和 B 站合集是同一种用法。Chromium 内核浏览器可跨会话保留文件权限，其余浏览器本次会话可播放。
...
- **卡片菜单（右上角 ⋯）**：视频库 / 收藏夹库的卡片右上角是「⋯」，里面是「重命名 /（恢复原名）/ 删除」——删除也收进来了，右上角不再单独放一个 ✕；右键卡片同样能打开。改名是就地编辑（回车保存、Esc 取消），只改本地显示，源站标题不动，「继续学习」里的同名卡片会一起同步。
